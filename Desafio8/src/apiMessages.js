...
class Messages {
    constructor(fileName){
        ...
        (async () => {
            await fs.promises.readFile(`./Desafio8/${this.fileName}`,'utf-8')
                .then( async (content) => {
                ...
                .catch(async (error) => {
                    const content = []
                    await fs.promises.writeFile(`./Desafio8/${this.fileName}`,JSON.stringify(content));
    ...
    async save(message) {
        try {
            ...
            let contenido = [];
            contenido =  JSON.parse(await fs.promises.readFile(`./Desafio8/${this.fileName}`,'utf-8'));
            contenido = [...contenido, message];
            await fs.promises.writeFile(`./Desafio8/${this.fileName}`,JSON.stringify(contenido));
        } catch (error) {
    ...
    async getAll () {
        try {
            const contenido =  JSON.parse(await fs.promises.readFile(`./Desafio8/${this.fileName}`,'utf-8'));
            return contenido;
        } catch (error) {
